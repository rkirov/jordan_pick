...
**Provenance / attribution.** This file is vendored (with light adaptation to
this project's classic-`import` style) from the Mathlib community pull request
[#36119](https://github.com/leanprover-community/mathlib4/pull/36119)
"feat(Topology/Covering): fundamental group of the circle is ℤ" by **Ruize Chen**,
released under the Apache-2.0 license (as all Mathlib contributions are). It is
included here only because that PR has not (yet) been merged into a Mathlib
release we depend on. All mathematical/code credit is Ruize Chen's; we use it as
the Phase-1 input (`π₁(S¹) ≅ ℤ`) to build the Brouwer fixed point theorem for the
plane, needed to discharge the `BrouwerFPT` interface of our Jordan curve theorem.

We prove that the fundamental group of the circle `AddCircle 1` (equivalently
`ℝ / ℤ`) is isomorphic to `ℤ`. The proof uses the covering space `ℝ → AddCircle 1`
and the homotopy lifting property.

## Main results

* `AddCircle.nLoop_surjective`, `AddCircle.nLoop_homotopic_iff`, `AddCircle.nLoop_add`
* `AddCircle.windingNumberIso : Multiplicative ℤ ≃* FundamentalGroup (AddCircle 1) 0`
...
theorem nLoop_surjective (ω : Path (0 : AddCircle (1 : ℝ)) 0) :
  ...
  have covMap_omega (s : I) :
    ...
    simp only [Function.comp_apply] at this; exact this
  exact ⟨{
